...
export async function GET(req: NextRequest) {
  ...
  try {
    // Filter users by role and event round scores, strictly excluding admins
    ...
    if (participants.length > 0) {
      const leaderboard = participants
        .filter((p) => !p.name?.toLowerCase().includes("admin") && p.role === "PARTICIPANT")
        .map((p) => {
      ...
      await redisClient.set(cacheKey, JSON.stringify(leaderboard), 10).catch(() => {});
  ...
  // Resilient fallback to 52 actual participants
  const { ACTUAL_PARTICIPANTS_LEADERBOARD } = await import("@/lib/platform-data");
  const fallbackLeaderboard = ACTUAL_PARTICIPANTS_LEADERBOARD.map((p, idx) => ({
    rank: idx + 1,
    participantId: p.participantId,
    name: p.name,
    college: p.college,
    totalScore: p.score,
    roundScores: { 1: Math.round(p.score * 0.4), 2: Math.round(p.score * 0.6) },
    explainUsed: false,
    codeUsed: false,
  }));
